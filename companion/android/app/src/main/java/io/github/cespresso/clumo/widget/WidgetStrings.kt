...
fun subtitleText(context: Context, snapshot: WidgetSnapshot): String =
    when (snapshot.subtitle) {
        WidgetSubtitle.None -> ""
        WidgetSubtitle.Alias, WidgetSubtitle.PatternName -> snapshot.subtitleText
        WidgetSubtitle.PomodoroDurations -> context.getString(
            R.string.widget_sub_pomodoro_durations,
            snapshot.subtitleArgA,
            snapshot.subtitleArgB,
        )
        WidgetSubtitle.TimerDuration -> context.getString(
            R.string.widget_sub_timer_duration,
            snapshot.subtitleArgA,
            snapshot.subtitleArgB,
        )
...
fun actionLabel(context: Context, action: WidgetAction): String =
    context.getString(
        when (action) {
